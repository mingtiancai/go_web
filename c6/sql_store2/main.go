...
func init() {
	var err error
	Db, err = sql.Open("postgres", "user=gaoming dbname=t password=tiancai sslmode=disable")
	if err != nil {
		panic(err)
	}
}

func (comment *Commemt) Create() (err error) {
	if comment.Post == nil {
...
func main() {
	...
	comment := Commemt{Content: "Good post!", Author: "Joe", Post: &post}
	comment.Create()
